...
void* mm_malloc (size_t size) {
  ...
  if(ptrFreeBlock == NULL) { // gets more space if needed
	requestMoreSpace(reqSize);
	ptrFreeBlock = searchFreeList(reqSize);
  }

  // set preceding block tag and header update
  precedingBlockUseTag = (ptrFreeBlock->sizeAndTags) & TAG_PRECEDING_USED;
  // get block size of the block
  ...
  if(blockSize - reqSize >= MIN_BLOCK_SIZE) { 
	...
	// pointer to split off block	
	BlockInfo* splitPtr = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
	// preceding tag set to true as halfof the block allocated	
	splitPtr->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED;
	// set tag used to false because block is going to be free	
	splitPtr->sizeAndTags &= ~0 << 1;
	// boundary tag
	*((size_t*)UNSCALED_POINTER_ADD(ptrFreeBlock, (blockSize - WORD_SIZE))) = blockSize - reqSize;	
	//block put into free list
	insertFreeBlock(splitPtr);
  } 
  //non-split case
  else {
	//pointer for the next block
	BlockInfo* nextBlock = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);
	// previous block used tag set	
	nextBlock->sizeAndTags |= TAG_PRECEDING_USED;

	ptrFreeBlock->sizeAndTags = ptrFreeBlock->sizeAndTags | TAG_USED;

  }
  //return pointer to beginning of free block and header
...
void mm_free (void *ptr) {
  ...
  *((size_t*) UNSCALED_POINTER_SUB(followingBlock, WORD_SIZE)) = blockInfo->sizeAndTags;
  ...
  //coalesce with other blocks in free list
