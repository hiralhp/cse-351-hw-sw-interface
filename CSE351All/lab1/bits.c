...
/*not x or not y = nand -> not nand = and*/
  return ~((~x)|(~y));
}
/*
 ...
 */
int thirdBits(void) {
/*1001001 = 49 in hex shifted 9 = 1001001000000000 adding 49 = 1001001001001001 and then add that again after shifting 18*/
  return (0x49 + ((0x49 << 9) << 18)) + (0x49 + (0x49 << 9));
  //return 0x49249249;
}
// Rating: 2
/*
 ...
 */
int sign(int x) {
/*if x = 0 both cases = 0, if x = + then right will be 1 and if x = - then left will be 1*/
    int s = x >> 31;
    int sig = (s & (~0)) + ((!s) & (!!x)); 
    return sig;
}
...
/*add z = x + y together. Shift x, y, and z to their MSB. if x MSB xor z MSB is 0 and 
 y MSB and z MSB is 0 Or x MSB xor y MSB is 1*/
  int z=x+y;
  int a=x>>31;
  int b=y>>31;
  int c=z>>31;
...
/*shift -1 to the left n shift above value to left p if above value xor x*/
    int mask = ~( ~0 << n ) << p;
